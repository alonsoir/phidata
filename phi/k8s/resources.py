...
class K8sResources(InfraResources):
    ...
    resources: Optional[List[Union[K8sResource, CreateK8sResource, ResourceGroup]]] = (
        None
    )
    ...
    def k8s_client(self) -> K8sApiClient:
        if self._api_client is None:
            self._api_client = K8sApiClient(
                context=self.context, kubeconfig_path=self.kubeconfig
            )
    ...
    def create_resources(
            ...
            for r in self.resources:
                if isinstance(r, ResourceGroup):
                    ...
                    if len(resources_from_resource_group) > 0:
                        for (
                            resource_from_resource_group
                        ) in resources_from_resource_group:
                            if isinstance(resource_from_resource_group, K8sResource):
                                ...
                                if (
                                    resource_from_resource_group.group is None
                                ...
                                resource_from_resource_group, CreateK8sResource
                            ):
                                _k8s_resource = resource_from_resource_group.create()
                                if _k8s_resource is not None:
                                    _k8s_resource.set_workspace_settings(
                                        workspace_settings=self.workspace_settings
                                    )
                                    if (
                                        _k8s_resource.group is None
                                        and self.name is not None
                                    ):
                    ...
                    if len(apps_from_app_group) > 0:
                        for app_from_app_group in apps_from_app_group:
                            if isinstance(app_from_app_group, K8sApp):
                                if (
                                    app_from_app_group.group is None
                                    and self.name is not None
                                ):
                                    app_from_app_group.group = self.name
                                if app_from_app_group.should_create(
                                    group_filter=group_filter
                                ):
                ...
                if len(app_resources) > 0:
                    for app_resource in app_resources:
                        if isinstance(
                            ...
                            if (
                                _k8s_resource is not None
                                and _k8s_resource.should_create(
                                    group_filter=group_filter,
                                    name_filter=name_filter,
                                    type_filter=type_filter,
                                )
        ...
        resources_to_create.sort(
            key=lambda x: K8sResourceInstallOrder.get(x.__class__.__name__, 5000)
        )
                        ...
                        for app_from_app_group in apps_from_app_group:
                            if isinstance(app_from_app_group, K8sApp):
                                if (
                                    ...
                                    app_from_app_group.group = self.name
                                if app_from_app_group.should_delete(
                                    group_filter=group_filter
                                ):
                ...
                if len(app_resources) > 0:
                    for app_resource in app_resources:
                        if isinstance(
                            ...
                            if (
                                _k8s_resource is not None
                                and _k8s_resource.should_delete(
                                    group_filter=group_filter,
                                    name_filter=name_filter,
                                    type_filter=type_filter,
                                )
                        ...
                        for app_from_app_group in apps_from_app_group:
                            if isinstance(app_from_app_group, K8sApp):
                                if (
                                    ...
                                    app_from_app_group.group = self.name
                                if app_from_app_group.should_update(
                                    group_filter=group_filter
                                ):
                ...
                if len(app_resources) > 0:
                    for app_resource in app_resources:
                        if isinstance(
                            ...
                            if (
                                _k8s_resource is not None
                                and _k8s_resource.should_update(
                                    group_filter=group_filter,
                                    name_filter=name_filter,
                                    type_filter=type_filter,
                                )
    ...
    def save_resources(
        ...
        for resource in final_k8s_resources:
            ...
            try:
                ...
                logger.error(
                    f"Failed to save {resource.get_resource_type()}: {resource.get_resource_name()}"
                )
                logger.exception(e)
                logger.error("Please fix and try again...")
